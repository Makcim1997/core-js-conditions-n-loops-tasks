...
function convertToRomanNumerals(num) {
  ...
  let str = '';
  const numberOfTens = (num - (num % 10)) / 10;
  const numberOfUnits = num % 10;

  if (num > 9) {
    for (let i = 0; i < numberOfTens; i += 1) {
      str += 'X';
    }
    str += `${romanNumerals[numberOfUnits]}`;
  } else {
    str += romanNumerals[numberOfUnits];
  }
  return str;
...
function sortByAsc(arr) {
  ...
  do {
    ...
    const index = array.indexOf(minNum);
...
function shuffleChar(str, iterations) {
  ...
  for (let j = 0; j < iterations; j += 1) {
    for (let i = 0; i < string.length; i += 1) {
      if (i % 2 === 0) {
        rightPartStr += string[i];
      } else {
        leftPartStr += string[i];
      }
    }

    string = rightPartStr + leftPartStr;
    leftPartStr = '';
    rightPartStr = '';
  }
  return string;
